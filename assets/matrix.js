/* Matrix logic for MiTRE-CR911 — externalized to avoid inline-script parsing issues */
(function () {
  ...
  // fallback playbook (if no MD file)
  ...
  async function loadPlaybookMD(id){
    try {
      const r = await fetch('playbooks/' + id + '.md', { cache:'no-store' });
  ...
  function createColumn(tactic){
    ...
    (tactic.techniques || []).forEach(tech => {
      ...
      c.onclick = async function () {
        ...
        // PRINT (no template literals, fully safe)
